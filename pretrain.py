...
import PIL
from network import PretrainSceneGraphModel
from data_loader import VisualGenomeDataset
...
def multi_gpu_pretrain(rank, world_size, config):
    ddp_setup(rank, world_size)
    pretrain(rank, config)

def pretrain(device, config):
    wandb.init(project="scenegraphgeneration", group="DDP", config=config)
    generator1 = torch.Generator().manual_seed(42)
    multi_gpu = config["multi_gpu"]

    #process dataset
    root_data_dir = "/gscratch/sciencehub/vision_datasets/visual_genome/processed"
    preproccess = torchvision.transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),
    dataset = VisualGenomeDataset(root_data_dir, 10, scale_factor=.25, transform=preproccess)
    

    train_dataset, test_dataset = torch.utils.data.random_split(dataset, [.998, .002], generator=generator1,)
    ...
    model = PretrainSceneGraphModel(10, dataset.num_object_labels, dataset.num_relationship_labels, **network_config)
    ...
    for epoch in tqdm(range(2, training_iterations)):
        ...
        # for batch in train_dataloader:
            ...
            if step % report_interval == 0:
                ...
                img_language_labels = [dataset.scene_graph_metadata["idx_to_label"][str(idx.cpu().item()+1)] for idx in node_labels_viz]
                img = draw_image(batch_rgb.cpu()[0], batch_object_bbox.cpu()[0], img_language_labels)
                img.save("test.png")
                img = wandb.Image(img)
            ...
            if (not multi_gpu or device == 0) and step % save_interval == 0:
...
def main(config: DictConfig) -> None:
    ...
    if config["multi_gpu"]:
        ...
        mp.spawn(multi_gpu_pretrain, args=(world_size, config,), nprocs=world_size)

    else:
        device = torch.device("cuda:0")
        pretrain(device, config)
