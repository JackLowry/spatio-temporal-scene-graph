...
from egtr.load_custom import load_cuda_kernels
from egtr.util import generalized_box_iou, sigmoid_focal_loss, nested_tensor_from_tensor_list, dice_loss
...
class DeformableDetrConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        eos_coefficient=0.1,
        focal_alpha=0.25,
        sequence_matching_threshold=0.2,
        num_rel_labels=7,
        **kwargs,
    ):
        ...
        # Sequence hungarian matcher
        self.sequence_matching_threshold = sequence_matching_threshold
        # Loss coefficients
...
class DeformableDetrFeatureExtractorWithAugmentor(DeformableDetrFeatureExtractor):
    ...
    def _resize(self, image, size, target=None, max_size=1333):
        """
...
@dataclass
class TemporalSceneGraphGenerationOutput(ModelOutput):
    """
...
class TemporalDeformableDetrForObjectDetection(DeformableDetrPreTrainedModel):
    def __init__(self, config: DeformableDetrConfig, fg_matrix=None):
        super().__init__(config)
        ...
        self.sequence_matcher = SequenceObjectMatcher(self.config.sequence_matching_threshold, config.d_model)
        ...
        if fg_matrix is not None:  # when training
    ...
    def forward(
        ...
        last_hidden_output = outputs["last_hidden_state"].reshape((batch_size, seq_len, self.num_queries, self.config.d_model))
            ...
            projected_q.append(
                proj_q(
                    q.transpose(1, 2).reshape(
                        [batch_size, seq_len, num_object_queries, self.config.d_model]
                    )
                    * unscaling
                )
            )
        ...
        decoder_attention_queries = decoder_attention_queries.unsqueeze(3).repeat(
            1, 1, 1, num_object_queries, 1, 1
        )
        decoder_attention_keys = decoder_attention_keys.unsqueeze(2).repeat(
            1, 1, num_object_queries, 1, 1, 1
        )
        ...
        subject_output = (
            self.final_sub_proj(last_hidden_output)
            .unsqueeze(3)
            .repeat(1, 1, 1, num_object_queries, 1)
        )
        object_output = (
            self.final_obj_proj(last_hidden_output)
            .unsqueeze(2)
            .repeat(1, 1, num_object_queries, 1, 1)
        )
        del last_hidden_output
        relation_source = torch.cat(
            [
                relation_source,
        ...
        if self.config.use_freq_bias:
            ...
            num_rel_labels = self.triplet_dist.shape[-1]

            pred_rel += bias_term.reshape((batch_size, seq_len, num_object_queries, num_object_queries, num_rel_labels))
        ...
        if labels is not None:
            ...
            criterion = TemporalEgtrLoss(
                matcher=matcher,
                num_classes=self.config.num_labels,
                num_object_queries=self.num_queries,
            ...
            # rel_gate: [bsz, seq_len, num_objects, num_objects, layer, 1]
            rel_gate = rel_gate.reshape(
                batch_size * seq_len * num_object_queries * num_object_queries, -1
            ).mean(0)
            log_layers = list(
        ...
        return TemporalSceneGraphGenerationOutput(
            loss=loss,
            loss_dict=loss_dict,
            logits=logits,
...
# taken from https://github.com/facebookresearch/detr/blob/master/models/detr.py
class TemporalEgtrLoss(nn.Module):
    """
    ...
    def loss_labels(self, outputs, targets, indices, matching_costs, num_boxes, log=True):
        ...
        for loss in self.losses:
            for seq_id in range(seq_len):
                ...
                if loss in losses.keys():
                    losses[loss] += self.get_loss(loss, seq_outputs, seq_targets, seq_indices, matching_costs, num_boxes)
                else:
                    losses.update(self.get_loss(loss, seq_outputs, seq_targets, seq_indices, matching_costs, num_boxes))
                ...
                for loss in self.losses:
                    ...
                    for seq_id in range(seq_len):
                        ...
                        l_dict = self.get_loss(
                            loss, seq_outputs, seq_targets, seq_indices, matching_costs, num_boxes
                        )
